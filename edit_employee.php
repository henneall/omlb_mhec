      	...
      	<div class="row">
        	<div class="col-lg-12">
		        ...
		        <div id="employee_table">
					<div class="col-lg-4" style="padding-left:0px;padding-right:5px;">
			      		<div class="dash-unit">
					  		<dtitle style="color:#b2c831">Update Employee</dtitle>
					  		<hr>
					  		<div style="padding:10px">
					  			<?php
					        	...
					        	<form method = "POST" action = "update_employee.php">
					        		<input type = "hidden" name = "id" value = "<?php echo $row1['employee_id']?>">
						  			<input type="text" name="employee_name" id="employee_name" class="form-control" placeholder="Last Name, First Name, Middle Initial" autocomplete="off" value = "<?php echo $row1['employee_name']?>" required>
						  			<span id="suggestion-employee_name"></span>
						  			<input type="text" name="position" id = "position" class="form-control" placeholder="Position" autocomplete="off" value = "<?php echo $row1['position']?>" required>
						  			...
						  			<select type="text" id="department" name='department' class = "form-control " placeholder="Department" required>
						  				...
						  				<?php						                 
						                  	$sql1 = mysqli_query($con,"SELECT * FROM department ORDER BY department_name ASC");
						                  	while($row = mysqli_fetch_array($sql1)) {
						                ?>
						  				<option value="<?php echo $row['department_id']?>" <?php echo (($row['department_id'] == $row1['department_id']) ? ' selected' : ''); ?>><?php echo $row["department_name"]?></option>
						  				<?php } ?>
						  			</select>
			                        <br>
	                        		<span><input type = "radio" name = "access"  id = "withaccess" value = "1" onclick="giveaccess()" <?php echo (($row1['access'] == '1') ? ' checked' : ''); ?> >&nbspWith Access</span>
	                        		<br>
	                        		<span><input type = "radio" name = "access" id = "woaccess" value = "0" onclick="giveaccess()" <?php echo (($row1['access'] == '0') ? ' checked' : ''); ?>>&nbspWithout Access</span><br><br>
	                        		<div id = "login">
						        ...
						        <tbody>	
						        	<?php 
						          	...
						          	<tr class="gradeA">
							           ...
							           <td>
							           		<center>
							           			<a href = "edit_employee.php?id=<?php echo $row['employee_id'];?>" class = "btn btn-primary btn-xs"><span class = "fa fa-edit"></span></a>
